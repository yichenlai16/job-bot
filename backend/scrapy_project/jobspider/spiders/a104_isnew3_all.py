...
#                 dict2 = {}
#                 dict2['jobcat'] = i['no']
#                 jobcode_list.append(dict2)

#     def start_requests(self):
#         for areacode in self.areacode_list:
#             yield FormRequest(url=self.start_url, method="GET", formdata=areacode, callback=self.jobcat_requests)

#     def jobcat_requests(self, response):
#         url = response.request.url
#         # script = response.xpath('//*[@id="job-jobList"]/script[5]/text()').get()
#         body = json.loads(response.body)
#         totalpage = body['data']['totalPage']
#         # totalpage= int(re.search('totalPage":(\d*)',str(script)).group(1))

#         if int(totalpage) >= int(150):
#             # print(url,totalpage)
#             for jobcat in self.jobcode_list:
#                 yield FormRequest(url=url, method="GET", formdata=jobcat, callback=self.page_requests)
#         else:
#             for page in range(1, totalpage+1):
#                 yield FormRequest(url+"&page="+str(page), callback=self.parse)
...
#         body = json.loads(response.body)
#         totalpage = body['data']['totalPage']
#         jobcat = body['data']['query']['jobcat']
#         area = body['data']['query']['area']

#         # area = int(re.search('"area":"(\d*)',str(script)).group(1))
#         # jobcat = int(re.search('"jobcat":"(\d*)',str(script)).group(1))

#         if int(totalpage) >= int(150):
#             extend_area = []
#             for i in self.resp.json()[0]['n']:
#                 if int(i['no']) == int(area):
#                     for i in i['n']:
#                         extend_area.append(i['no'])
#             for area in extend_area:
#                 yield FormRequest(self.start_url+"&area="+str(area)+'&jobcat='+str(jobcat)+'&isnew=0', callback=self.ro_requests, dont_filter=False)

#         for page in range(1, totalpage+1):
#             yield FormRequest(url+"&page="+str(page)+'&isnew=0', dont_filter=False, callback=self.parse)

#     def ro_requests(self, response):
#         url = response.request.url
#         body = json.loads(response.body)
#         totalpage = body['data']['totalPage']
#         if int(totalpage) >= int(150):
#             # print(url,totalpage)
#             for ro in range(1, 5):
#                 yield FormRequest(url=url+"&ro="+str(ro)+'&isnew=0', method="GET", callback=self.extended_requests)
#         else:
#             for page in range(1, totalpage+1):
#                 yield FormRequest(url+"&page="+str(page)+'&isnew=0', callback=self.parse)

#     def extended_requests(self, response):
#         url = response.request.url
#         body = json.loads(response.body)
#         totalpage = body['data']['totalPage']
#         for page in range(1, totalpage+1):
...
#             source_company_sn = str(
#                 source_company_url.split('/')[-1].split('?')[0])
...
#             yield FormRequest(('https://www.104.com.tw/job/ajax/content/'+source_job_sn),
#                               meta=meta,
...
#         jobCategory = list([ x['description'] for x in job_data['jobDetail']['jobCategory']])
...
#         roleDesc = list([ x['description'] for x in job_data['condition']['acceptRole']['role']])
...
#         language = [i['language'] +', '+ i['ability'] for i in job_data['condition']['language']]
#         # language = job_data['condition']['language']
#         localLanguage = [i['language'] + i['ability'] for i in job_data['condition']['localLanguage']]
...
#         yield FormRequest('https://www.104.com.tw/company/ajax/content/'+meta['source_company_sn'],
#                           meta=meta,
#                           callback=self.getCompany,dont_filter=True)
